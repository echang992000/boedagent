...
- `examples/cases/bmp4_gradient/prompts/multireceptor_prior_prompt.md`
...
1. Synthesize the BMP4 multireceptor-family literature prior strictly from the
   local BMP4 corpus and files above.
2. Follow the lower-level multireceptor prompt exactly.
3. Write the final JSON artifact to:
   - `artifacts/bmp4_gradient/priors/multireceptor_literature_prior.json`
...
Important model-family constraint:

- Do not synthesize receptor-abundance priors from literature.
- Keep abundance / `Rs` / qPCR handling out of the output JSON.
