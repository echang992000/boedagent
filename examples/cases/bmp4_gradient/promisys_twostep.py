...
DIMER_COMPLEX_NAMES = tuple(
    f"{LIGAND_NAME}_{type_i}"
    for type_i in TYPE_I_RECEPTORS
)
TRIMER_COMPLEX_NAMES = tuple(
    f"{LIGAND_NAME}_{type_i}_{type_ii}"
    for type_i in TYPE_I_RECEPTORS
    for type_ii in TYPE_II_RECEPTORS
)
COMPLEX_NAMES = TRIMER_COMPLEX_NAMES
BINDING_PARAMETER_NAMES = tuple(f"K_{name}" for name in DIMER_COMPLEX_NAMES) + tuple(
    f"K_{name}" for name in TRIMER_COMPLEX_NAMES
)
EFFICIENCY_PARAMETER_NAMES = tuple(f"e_{name}" for name in TRIMER_COMPLEX_NAMES)
...
def _progress(message: str) -> None:
    print(f"[promisys_twostep] {message}", flush=True)
...
def run_promisys_twostep_workflow(
    *,
    joint_data: Any,
    run_dir: str | Path,
    ...
    if np is None:
        raise RuntimeError("The promisys_twostep workflow requires numpy.")
    import torch

    _progress("Starting BMP4 promisys two-step workflow")
    ...
    theta_prior = _build_twostep_theta_prior(literature_prior)
    if theta_prior["mode"] == "expert_mapped":
        ...
        _progress(
            "Using mapped expert prior for twostep SNPE K parameters: "
            + ", ".join(mapped)
        )
    else:
    ...
    eig_result["eig_steps_ignored_for_promisys_twostep"] = True
    ...
    _progress("Promisys two-step workflow complete")
    return {
...
def _build_twostep_theta_prior(literature_prior: Any | None) -> dict[str, Any]:
    ...
    for name in BIOPHYSICAL_PARAMETER_NAMES:
        receptor_name = _twostep_receptor_prior_name_for_parameter(name)
    ...
    return {
        ...
        "mapping": (
            "Twostep dimer affinities K_BMP4_typeI use kd_typeI priors; "
            "twostep trimer affinities K_BMP4_typeI_typeII use kd_typeII priors. "
...
def _twostep_receptor_prior_name_for_parameter(parameter_name: str) -> str | None:
...
def split_theta_raw(theta_raw: Any) -> tuple[Any, Any, Any]:
    ...
    if theta.shape[-1] != THETA_DIM:
        raise ValueError(f"Expected theta dimension {THETA_DIM}, got {theta.shape[-1]}.")
    binding_count = len(BINDING_PARAMETER_NAMES)
    efficiency_count = len(EFFICIENCY_PARAMETER_NAMES)
    return (
        theta[..., :binding_count],
        theta[..., binding_count : binding_count + efficiency_count],
...
def simulate_promisys_twostep_raw(
    *,
...
def simulate_promisys_twostep_paired_raw(
    *,
    bmp4_concentrations: Any,
    receptors: Any,
    theta_norm: Any,
...
def problem_description(problem_summary: str, receptor_names: Sequence[str]) -> str:
    return (
        f"{problem_summary} Candidate model family: promisys two-step BMP4 trimeric "
        f"model over receptors {', '.join(receptor_names)}. Infer two dimer "
        "affinities, six trimer affinities, six phosphorylation efficiencies, and one normalized "
    ...
    parameters = [
        {
            "name": name,
            "description": (
                "Normalized two-step BMP4 dimer affinity"
                if name in {f"K_{complex_name}" for complex_name in DIMER_COMPLEX_NAMES}
                else "Normalized two-step BMP4 trimer affinity"
            )
            if name.startswith("K_")
            else "Normalized two-step BMP4 phosphorylation efficiency",
        }
        for name in BIOPHYSICAL_PARAMETER_NAMES
    ]
    parameters.append(
...
def _validate_joint_data(joint_data: Any) -> None:
    ...
    if receptor_names != TARGET_RECEPTOR_NAMES:
        raise ValueError(
            "promisys_twostep requires receptor order "
...
def _require_promisys(lfiax_root: str | Path | None) -> Any:
    ...
    if missing:
        raise RuntimeError(
            "promisys_twostep requires optional simulator dependencies that are "
...
def _simulate_snpe_training_data(
    *,
    ...
    for simulation_index in range(total):
        ...
        y_raw = simulate_promisys_twostep_raw(
    ...
    return (
        ...
        np.asarray(targets, dtype=np.float32),
        {
            ...
            "dimer_complex_names": list(DIMER_COMPLEX_NAMES),
...
def _run_joint_multicontext_boed(
    *,
    ...
    eig_result = {
        ...
        "eig_steps_ignored_for_promisys_twostep": True,
        "infonce_lambda": float(infonce_lambda),
...
def _simulate_joint_boed_batch(
    *,
    ...
    for cell_index, cell_line in enumerate(joint_data.cell_lines):
        ...
        receptors_raw = np.asarray(joint_data.q_obs[cell_index], dtype=np.float32)
        y_raw = simulate_promisys_twostep_paired_raw(
    ...
    *,
    ...
    for cell_index, cell_line in enumerate(joint_data.cell_lines):
        ...
        for theta_index, theta in enumerate(chosen):
            mu_raw = simulate_promisys_twostep_raw(
...
def _write_promisys_artifacts(
    *,
    ...
    save_joint_posterior_predictive_plot(
        ...
        title="Posterior predictive BMP4 dose-response: promisys two-step",
    )
    ...
    save_eig_optimization_plot(
        eig_result["history"],
        eig_plot_path,
        title="Next-design EIG optimization: promisys two-step",
    )
    return {
...
def _samples_payload(
    ...
    *,
    ...
    return {
        ...
        "receptor_names": list(TARGET_RECEPTOR_NAMES),
        "theta_coordinate": (
            "First 14 coordinates are Gaussianized log-uniform biophysical "
...
def _save_theta_comparison_plot(
    ...
    *,
    ...
    fig.suptitle(
        f"promisys_twostep parameter comparison ({cell_line}, {'positive' if positive else 'normalized'} scale)"
    )
    fig.tight_layout()
